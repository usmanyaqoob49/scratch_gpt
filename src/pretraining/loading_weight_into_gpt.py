...
def loads_weight_into_gpt(gpt_model, params):
    ...
    for b in range(len(params['blocks'])):
        ...
        gpt_model.transformer_blocks[b].attention.W_key.bias= assign(
            gpt_model.transformer_blocks[b].attention.W_key.bias, k_b
        )
        gpt_model.transformer_blocks[b].attention.W_value.bias= assign(
            gpt_model.transformer_blocks[b].attention.W_value.bias, v_b
        )
         #getting attention output projection weights and biases
        gpt_model.transformer_blocks[b].attention.out_proj.weight = assign(
            gpt_model.transformer_blocks[b].attention.out_proj.weight, 
            params["blocks"][b]["attn"]["c_proj"]["w"].T)
        gpt_model.transformer_blocks[b].attention.out_proj.bias = assign(
            gpt_model.transformer_blocks[b].attention.out_proj.bias, 
